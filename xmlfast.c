...
static struct entityref entities; // tree of entity candidates. generated by calculate
...
void calculate(char *prefix, unsigned char offset, entity *strings, struct entityref *ents) {
	...
	for (i = 0; i < 255; i++) { if (counts[i]) total++; }
				...
				switch (*p) {
					...
					case '?':
						...
						if (*p == '?' && *(p+1) == '>') {
							...
							if (context->cb.die)
								context->cb.die("Processing instruction not terminated");
							goto fault;
						}
					case '/': // </node>
						...
						if (search) {
							...
							if (context->depth == 0) {
								if (context->cb.warn)
									context->cb.warn("Need to close tag upper than root. Ignored");
								goto next;
							}
							if (strncmp(context->chain->name, at, len) == 0) {
								...
								while( seek > context->root ) {
									...
									if (strncmp(seek->name, at, len) == 0) {
										if (context->cb.warn)
											context->cb.warn("Found early opened node %s",seek->name);
							...
							if (context->cb.die)
								context->cb.die("close tag not terminated");
							goto fault;
						}
					default: //<node...>
						state = 0;
							...
							switch(state) {
								case 0:
									...
									if (context->depth + 1 > context->chain_size) {
										...
										context->root = saferealloc(context->root, sizeof(xml_node) * context->chain_size);
								...
								case 2:
									while(state == 2) {
										switch(*p) {
											...
											default  :
												if (context->cb.die)
													context->cb.die("Bad char '%c' at the end of tag",*p);
												goto fault;
										}
									}
									context->state = CONTENT_WAIT;
